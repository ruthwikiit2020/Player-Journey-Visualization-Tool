...
- Accurate minimap layering (world -> minimap pixel mapping)
- Human vs bot paths (distinct styles)
- Event markers (kills, deaths, loot, storm deaths)
- Filters by map/date/match/player type
- Match timeline playback + event ticks
- Heatmap overlays (kill zones, death zones, traffic, loot density)

### Local dev (if you want to run it yourself)
Serve the folder over HTTP (not `file://`), e.g.:
